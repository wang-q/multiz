/* $Id: seq.h 142 2008-11-12 18:55:23Z rico $ */
...
enum {
    SEQ_TYPE_GUESS = 0, SEQ_TYPE_FASTA
};

typedef struct seq_file {
    uchar *seq;
    int slen;
    /* bytes in seq, not including '\0' */
    int from;
    /* 1 based */

    FILE *fp;
    int type; // guess, fasta, ...
    int flags;
    int count;
    /* how many contigs we have already read */
    long offset;
    /* the starting offset of the contig we just read */

    char *maskname;
    char *fname;
    char *header;
    int hlen;
    /* bytes in header, not including '\0' */
    struct seq_file *contigs;
    int numContigs;
    char flipped;
}
        SEQ;
